...
export default function BusinessList({
	...
	const [selectedBusinessCurrency, setSelectedBusinessCurrency] = useState(null);
	...
	// Handlers
	const handleShowAddMenuItem = (menuId, sectionId, currency) => {
		setSelectedMenuId(menuId);
		setSelectedSection(sectionId);
		setSelectedBusinessCurrency(currency);

		const business = businesses.find(b => b.menus?.some(m => m.id === menuId));
		if (business) {
			setSelectedBusinessCategoryId(business.businessCategoryId);
		}

		setShowAddMenuItem(prev => ({ ...prev, [menuId]: true }));
	};

	const handleShowEditMenuItem = (item, menuId, currency, sectionId) => {
	...
	const handleItemAdded = (menuId, newItem) => {
		onRefresh();
		handleCancelAddMenuItem(menuId);
	};
	...
	const handleDeleteMenuItem = async (menuId, itemId, itemName, businessId) => {
		if (window.confirm(`¿Estás seguro de que quieres eliminar "${itemName}"?`)) {
			try {
				await menuService.deleteMenuItem(businessId, menuId, itemId);
				onRefresh();
			} catch (error) {
	...
	const handleRefreshBusinessData = () => {
		onRefresh();
	};

	const handleLoadFlyer = (flyer, businessId, menuId) => {
		setShowSavedFlyers(null);
		setShowPromotionalFlyer({ businessId, menuId, savedFlyer: flyer });
	};
								...
								<div key={menu.id} className="mb-4 sm:mb-6">
									<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
										...
										<div className="flex flex-wrap gap-2 w-full sm:w-auto">
											...
											{onMenuScanner && (
												<button
													onClick={() => handleAiScanClick(business.id, menu.id)}
													className="neo-btn neo-btn-secondary text-xs sm:text-sm flex items-center justify-center flex-1 sm:flex-initial"
														...
														<div className="flex justify-between items-center">
															...
															<button
																onClick={() => {
																	...
																	handleAddMenuItemClick(menu.id, section.id, business.defaultCurrency);
																}}
